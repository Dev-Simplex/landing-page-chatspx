...
const AnimatedChatDemo = ({ isActive }: { isActive: boolean }) => {
  const [messages, setMessages] = useState([
    { text: "Oi! Como posso ajudar hoje?", isBot: true, visible: false },
    { text: "Quero falar sobre meu pedido", isBot: false, visible: false },
    { text: "Claro! Pode informar o número do pedido?", isBot: true, visible: false },
  ])
  ...
  useEffect(() => {
    ...
    const scenarios = [
      [
        { text: "Oi! Como posso ajudar hoje?", isBot: true },
        { text: "Quero falar sobre meu pedido", isBot: false },
        { text: "Claro! Pode informar o número do pedido?", isBot: true },
      ],
      [
        { text: "Olá! Estou disponível 24/7 para ajudar.", isBot: true },
        { text: "Vocês atendem no fim de semana?", isBot: false },
        { text: "Posso verificar os horários disponíveis para você.", isBot: true },
      ],
      [
        { text: "Boa noite! Como posso ajudar?", isBot: true },
        { text: "Preciso de ajuda com preços", isBot: false },
        { text: "Posso enviar as informações agora mesmo!", isBot: true },
      ],
    ]
          ...
          >
            <div
              className={`max-w-[80%] px-3 py-1.5 rounded-full text-xs ${
                msg.isBot ? "bg-slate-200 text-slate-700" : "bg-green-600 text-white"
              }`}
            >
              {msg.text}
            </div>
...
const AnimatedPhoneDemo = ({ isActive }: { isActive: boolean }) => {
  ...
  return (
    <div className="bg-slate-50 rounded-lg p-4 h-32 flex items-center justify-center relative">
      <div className="absolute top-2 right-2 text-xs text-slate-500 font-medium">Chamadas: {callCount + 1}</div>
      <div className="relative">
        <div
          ...
          } ${callState === "answered" ? "bg-green-600" : ""}`}
        >
    ...
    <div className="bg-slate-50 rounded-lg p-4 h-32">
      ...
      {booked && (
        <div className="mt-2 text-xs text-green-600 font-medium animate-fade-in">
          ✓ Agendamento confirmado para o dia 15
        </div>
      )}
    </div>
  )
...
const AnimatedEmailDemo = ({ isActive }: { isActive: boolean }) => {
  const [emails, setEmails] = useState([
    { subject: "Dúvida sobre atendimento", status: "unread" },
    { subject: "Solicitação de suporte", status: "unread" },
    { subject: "Preciso de cotação", status: "unread" },
  ])
    ...
    <div className="bg-slate-50 rounded-lg p-4 h-32 overflow-hidden">
      <div className="space-y-2">
        {emails.map((email, i) => (
          ...
          >
            <div className={`w-2 h-2 rounded-full ${email.status === "replied" ? "bg-green-500" : "bg-green-600"}`} />
...
const AnimatedLeadsDemo = ({ isActive }: { isActive: boolean }) => {
  const [leads, setLeads] = useState([
    { name: "Carla M.", score: 0, qualified: false },
    { name: "João D.", score: 0, qualified: false },
    { name: "Marcos R.", score: 0, qualified: false },
  ])
            ...
            <div className="flex-1 bg-slate-200 rounded-full h-2">
              <div
                className={`h-2 rounded-full transition-all duration-500 ${
                  lead.qualified ? "bg-green-500" : "bg-green-600"
                }`}
...
const AnimatedIntegrationsDemo = ({ isActive }: { isActive: boolean }) => {
  const [connections, setConnections] = useState([
    ...
    { name: "Dialogflow", connected: false },
...
const features = [
  {
    ...
    description:
      "Centralize conversas do site, WhatsApp, Instagram, e-mail e mais em um único inbox com histórico completo.",
    demo: AnimatedChatDemo,
    size: "large",
  },
  {
    title: "Agente de IA (Captain)",
    description: "Automatize respostas iniciais, resolva dúvidas comuns e direcione conversas para o time certo.",
    demo: AnimatedPhoneDemo,
    size: "medium",
  },
  {
    title: "Horários e auto-respostas",
    description: "Defina horários de atendimento, regras e respostas automáticas para manter SLA e expectativas.",
    demo: AnimatedCalendarDemo,
    size: "medium",
  },
  {
    title: "E-mail integrado e respostas prontas",
    description: "Responda mais rápido com respostas prontas, automações e encaminhamento inteligente.",
    demo: AnimatedEmailDemo,
    size: "large",
  },
  {
    title: "Contatos e segmentação",
    description: "Gerencie perfis, tags e atributos personalizados para campanhas e atendimento contextual.",
    demo: AnimatedLeadsDemo,
    size: "medium",
  },
  {
    title: "Integrações essenciais",
    description: "Conecte o CHAT - SPX com Slack, Shopify, Dialogflow, Linear e webhooks.",
    ...
    <section id="features" ref={sectionRef} className="relative z-10">
      <div className="bg-white rounded-t-[3rem] pt-16 sm:pt-24 pb-16 sm:pb-24 px-4 relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto relative">
          ...
          >
            <div className="inline-flex items-center px-4 py-2 rounded-full bg-emerald-50 border border-emerald-100 text-emerald-700 text-sm font-medium mb-6">
            ...
            <h2 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-bold text-slate-900 text-balance mb-4 sm:mb-6">
              Recursos que unem canais, equipe e{" "}
              <span className="bg-gradient-to-r from-green-600 to-emerald-400 bg-clip-text text-transparent">IA</span>
            </h2>
            <p className="text-base sm:text-lg md:text-xl text-slate-600 max-w-3xl mx-auto font-light leading-relaxed">
              Centralize conversas, automatize fluxos e acompanhe tudo com relatórios e visibilidade em tempo real.
            </p>
          </div>
            ...
            {features.map((feature, index) => (
              ...
              >
                <div className="bg-white rounded-2xl p-6 sm:p-8 h-full shadow-lg hover:shadow-2xl transition-all duration-500 hover:-translate-y-2 border border-slate-200 hover:border-slate-300">
                  ...
                  <h3 className="text-xl sm:text-2xl font-bold text-slate-900 mb-4 group-hover:text-green-600 transition-colors duration-300">
                    {feature.title}
                  </h3>
